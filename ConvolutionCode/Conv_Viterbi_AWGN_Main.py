...
# Plot Data
Eb_data = np.linspace(1.9, 5.5, ITERATIONS)
...
for iter in range(ITERATIONS):
    ...
    for trial in range(TRIALS):
        noise = np.reshape(np.random.normal(0, 1.0, size=BLOCK_LENGTH * 2), (BLOCK_LENGTH, 2))
        coded_observation += noise
        # for i in range(BLOCK_LENGTH):
        #     for j in range(2):
        #         coded_observation[i][j] += random.gauss(0, 1.0)
    ...
    print(f"Eb/No = {Eb_data[iter]:.6f} BER soft = {BER_data_soft[iter]:.6f}")
    print(f"Eb/No = {Eb_data[iter]:.6f} BER hard = {BER_data_hard[iter]:.6f}")
...
plt.legend(["Soft Desicion", "Hard Desicion"])
plt.ylim(10 ** (-6), 10 ** (-1))
plt.show()
